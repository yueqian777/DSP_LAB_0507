...
$js = Join-Path $PSScriptRoot "dss\dss_dynamic_clarity_board.js"
$haltJs = Join-Path $PSScriptRoot "dss\dss_halt_board.js"
$generator = Join-Path $PSScriptRoot "generate_dynamic_clarity_audio.py"
$analyzer = Join-Path $PSScriptRoot "analyze_dynamic_clarity_raw.py"
...
if ([string]::IsNullOrWhiteSpace($Program)) {
    throw "Program is required unless AnalyzeOnly is selected"
}
foreach ($required in @(
    $Program,$dss,$js,$haltJs,$generator,$analyzer,$ccxml
)) {
...
foreach ($file in $manifest.files) {
    $waves[$file.name] = Join-Path $audioDir "$($file.name).wav"
}
...
if (-not (Test-Path -LiteralPath $serialLog)) {
    throw "UART capture was not created"
}
$serialText = Get-Content -Raw -LiteralPath $serialLog
if ($serialText -notmatch [Regex]::Escape("P33 BUILD $ExpectedSha")) {
    throw "UART log does not contain the expected P33 BUILD record"
