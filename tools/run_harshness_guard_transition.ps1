...
$js = Join-Path $PSScriptRoot "dss\dss_harshness_guard_transition.js"
$generator = Join-Path $PSScriptRoot "generate_harshness_guard_audio.py"
$analyzer = Join-Path $PSScriptRoot "analyze_harshness_guard_transition.py"
...
if ($LASTEXITCODE -ne 0 -or
    ...
    throw "Harshness Guard transition capture requires a clean worktree"
}

if ([string]::IsNullOrWhiteSpace($ResultDir)) {
    $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $ResultDir = Join-Path $env:TEMP `
        "DSP_LAB_0507\harshness_guard_transition\$stamp"
}
...
$defines = @(
    ...
    "--define=EQ_ENABLE_HARSHNESS_GUARD_TRANSITION_CAPTURE=1",
...
if (-not $SkipBuild) {
    ...
    $objects = @(
        (Join-Path $root "Debug\Code\User\user_dsp\user_harshness_guard.obj"),
...
try {
    ...
    if ($process.ExitCode -ne 0) {
        $boardPath = Join-Path $resultPath `
            "harshness_guard_transition_board.json"
        $completed = $false
    ...
    Remove-Item Env:DSP_TEST_ROOT,Env:DSP_TEST_CCXML,
        ...
        Env:DSP_TEST_EXPECTED_SHA `
        -ErrorAction SilentlyContinue
}

$boardPath = Join-Path $resultPath "harshness_guard_transition_board.json"
...
[pscustomobject]@{
    ...
    report = Join-Path $resultPath "harshness_guard_transition_artifact.json"
    timing = $boardPath
