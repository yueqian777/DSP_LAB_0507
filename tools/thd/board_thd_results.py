...
from analyze_thd import analyze_wav, write_artifacts, write_pcm16_mono
...
def write_board_summary(output_dir: Path, rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        ...
        "",
        "The deterministic PCM16 stimulus was generated by the C6748, captured before "
        "processing, passed through the real C6748 WOLA implementation, and exported "
        "from DDR through JTAG. No PC playback or recording device was used.",
        "",
...
def analyze_outputs(
    ...
    for frequency_hz in frequencies:
        frequency_dir = output_dir / f"{frequency_hz}Hz"
        input_raw = frequency_dir / "board_input_full.pcm16le"
        output_raw = frequency_dir / "output_full.pcm16le"
        full_input = decode_c6748_raw(input_raw)
        full_output = decode_c6748_raw(output_raw)
        input_wav = frequency_dir / f"board_input_{tone_filename(frequency_hz)}"
        aligned = full_output[WOLA_DELAY : WOLA_DELAY + INPUT_SAMPLES].copy()
        output_wav = frequency_dir / f"board_{tone_filename(frequency_hz)}"
        write_pcm16_mono(input_wav, 50_000, full_input[:INPUT_SAMPLES])
    ...
    metadata: dict[str, Any] = {
        ...
        "input_generation": "C6748_100_SAMPLE_PERIODIC_SINE_MINUS12DBFS",
