/* Objective-only Project 3.3 Harshness Guard board validation. */
...
function boardSnapshot() {
    return {
        ...
        guard_processing: numberValue("EQ_DebugHarshnessGuardProcessingActive"),
        guard_brightness_db: numberValue("EQ_DebugHarshnessGuardBrightnessDb"),
        guard_presence_db: numberValue("EQ_DebugHarshnessGuardPresenceDb"),
        guard_excess_db: numberValue("EQ_DebugHarshnessGuardExcessDb"),
        guard_rms_dbfs: numberValue("EQ_DebugHarshnessGuardRmsDbfs"),
        guard_requested: numberValue("EQ_DebugHarshnessGuardRequestedLevel"),
        guard_applied: numberValue("EQ_DebugHarshnessGuardAppliedLevel"),
        guard_pending: numberValue("EQ_DebugHarshnessGuardPendingLevel"),
        guard_requested_gain_db:
            numberValue("EQ_DebugHarshnessGuardRequestedGainDb"),
        guard_applied_gain_db:
            numberValue("EQ_DebugHarshnessGuardAppliedGainDb"),
        guard_transition: numberValue("EQ_DebugHarshnessGuardTransitionActive"),
        guard_progress: numberValue("EQ_DebugHarshnessGuardTransitionProgress"),
        guard_reason: numberValue("EQ_DebugHarshnessGuardReason"),
        guard_decisions: numberValue("EQ_DebugHarshnessGuardDecisionCount"),
        guard_level_changes:
            numberValue("EQ_DebugHarshnessGuardLevelChangeCount"),
        guard_transitions:
            numberValue("EQ_DebugHarshnessGuardTransitionCount"),
        guard_invalid_releases:
            numberValue("EQ_DebugHarshnessGuardInvalidReleaseCount"),
        guard_last_cycles: numberValue("EQ_DebugHarshnessGuardLastCycles"),
        guard_max_cycles: numberValue("EQ_DebugHarshnessGuardMaxCycles"),
        guard_saturation:
            numberValue("EQ_DebugHarshnessGuardSaturationCount"),
        guard_nonfinite:
            numberValue("EQ_DebugHarshnessGuardNonFiniteCount"),
        four_way_mask: numberValue("EQ_DebugFourWayTransitionMask"),
        four_way_overlap_count:
            numberValue("EQ_DebugFourWayTransitionOverlapCount"),
        four_way_first_frame:
            numberValue("EQ_DebugFourWayTransitionFirstFrame"),
        four_way_arm_mode:
            numberValue("EQ_DebugFourWayTransitionArmMode"),
        four_way_arm_count:
            numberValue("EQ_DebugFourWayTransitionArmCount"),
...
function checkSafety(state, label) {
    ...
    requireCondition(state.clip == 0 && state.smart_saturation == 0 &&
                     ...
                     state.guard_saturation == 0 &&
                     state.guard_nonfinite == 0,
    ...
    requireCondition(state.guard_applied_gain_db <= 0.001 &&
                     state.guard_applied_gain_db >= -2.001,
                     label + ": Harshness Guard gain is outside bounds");
}
...
function requireAdjacentGuard(values, label) {
    var index, before, after;
    for (index = 0; index < values.length; index++) {
        requireCondition(values[index].guard_requested >= 0 &&
                         values[index].guard_requested <= 3 &&
                         values[index].guard_applied >= 0 &&
                         values[index].guard_applied <= 3,
                         label + ": level exceeded MEDIUM ceiling");
        requireCondition(values[index].guard_applied_gain_db <= 0.001 &&
                         values[index].guard_applied_gain_db >= -2.001,
                         label + ": gain left the bounded attenuation range");
        if (index > 0) {
            before = values[index - 1].guard_applied;
            after = values[index].guard_applied;
            requireCondition(Math.abs(after - before) <= 1,
                             label + ": applied level skipped an adjacent level");
        }
    }
}

function requireGuardReleaseConfirmations(start, values, label) {
    var startTransitions = start.guard_transitions;
    var startDecision = start.guard_decisions;
    var observedTransitions = startTransitions;
    var index, state, releaseSteps;
    for (index = 0; index < values.length; index++) {
        state = values[index];
        if (state.guard_transitions > observedTransitions) {
            releaseSteps = state.guard_transitions - startTransitions;
            requireCondition(state.guard_decisions - startDecision >=
                             2 * releaseSteps,
                             label + ": cumulative release used fewer than " +
                             "two decisions per level");
            observedTransitions = state.guard_transitions;
        }
    }
}

function expectedGuardLevel(state) {
    var level;
    if (state.guard_rms_dbfs < -45.0 || state.guard_brightness_db < 0.0 ||
        state.guard_excess_db <= 6.0) return 0;
    level = Math.floor(((state.guard_excess_db - 6.0) / 12.0) * 3.0 + 0.5);
    if (level < 0) level = 0;
    if (level > 3) level = 3;
    return level;
}

function configureHarshnessBase() {
    write("EQ_DebugDiagPath = 4");
    write("EQ_DebugMode = 0");
    write("EQ_DebugAnalyzerEnabled = 1");
    write("EQ_DebugSmartBassStrength = 2");
    write("EQ_DebugDynamicClarityStrength = 2");
    write("EQ_DebugHarshnessGuardStrength = 2");
    write("EQ_DebugSmartBassEnabled = 0");
    write("EQ_DebugDynamicClarityEnabled = 0");
    write("EQ_DebugHarshnessGuardEnabled = 0");
}

function runHarshnessObjective() {
    var result = {}, build, state, before, after, series, release;
    var mappingNames = ["harshness_sweep_2db", "harshness_sweep_7db",
                        "harshness_sweep_10db", "harshness_sweep_14db",
                        "harshness_sweep_18db"];
    var mappingTargets = [2.0, 7.0, 10.0, 14.0, 18.0];
    var mapping = [], jitter = [], index, expected, previous = -1;
    var maxSmart = 0, maxClarity = 0, maxGuard = 0;
    var combinedBefore, combinedAfter, combinedSeries = [];
    var events = [], fourBass = false, fourVocal = false;
    var bassRequested = false, vocalRequested = false;
    var bassCapture = null, vocalCapture = null, start, finalState;
    var releaseBetween, continuousStart, continuousFinal;
    var hostStartNs, hostEndNs, hostElapsed, dspAudioSeconds;
    ...
    result.boot = state;
    result.subjective_observation = "NOT_PERFORMED";
    result.external_analog_metrics = "UNMEASURED";
    configureHarshnessBase();
    ...
    state = boardSnapshot();
    checkSafety(state, "A runtime OFF");
    requireCondition(state.guard_applied == 0 && state.guard_processing == 0 &&
                     state.guard_transition == 0,
                     "A Guard OFF path was not identity");
    result.runtime_off_60s = state;
    result.runtime_off_capture = capturePair("runtime_off_identity");
    ...
    checkSafety(state, "B Presence identity");
    checkAnalyzer(state, 2, "B Presence identity");
    requireCondition(state.guard_requested == 0 && state.guard_applied == 0 &&
                     state.guard_transition == 0,
                     "B Presence input triggered Harshness Guard");
    result.presence_identity = state;
    result.presence_capture = capturePair("presence_identity");
    ...
    write("EQ_DebugAnalyzerResetRequest = 1");
    runFor(4000);
    result.brightness_off = boardSnapshot();
    checkSafety(result.brightness_off, "C Brightness OFF");
    result.brightness_off_capture = capturePair("brightness_off");
    write("EQ_DebugHarshnessGuardEnabled = 1");
    series = readSeries(55, 100);
    requireAdjacentGuard(series, "C Brightness attack");
    runFor(1000);
    state = boardSnapshot();
    checkSafety(state, "C Brightness trigger");
    checkAnalyzer(state, 3, "C Brightness trigger");
    requireCondition(state.guard_brightness_db >= 0.0 &&
                     state.guard_excess_db > 6.0 &&
                     state.guard_requested == 3 && state.guard_applied == 3 &&
                     state.guard_transition == 0,
                     "C Brightness did not settle at MEDIUM ceiling");
    result.brightness_attack = series;
    result.brightness_on = state;
    result.brightness_on_capture = capturePair("brightness_on");

    for (index = 0; index < mappingNames.length; index++) {
        requestAudio(mappingNames[index]);
        write("EQ_DebugAnalyzerResetRequest = 1");
        runFor(5000);
        state = boardSnapshot();
        checkSafety(state, "D mapping " + mappingNames[index]);
        expected = expectedGuardLevel(state);
        requireCondition(Math.abs(state.guard_excess_db - mappingTargets[index]) <=
                         2.0,
                         "D measured excess missed calibrated target");
        requireCondition(state.guard_requested == expected &&
                         state.guard_applied == expected &&
                         state.guard_applied >= previous,
                         "D measured mapping did not match the control formula");
        previous = state.guard_applied;
        mapping.push({stimulus: mappingNames[index],
                      target_harshness_db: mappingTargets[index],
                      expected_level: expected, state: state});
    }
    requireCondition(mapping[0].state.guard_applied == 0 &&
                     mapping[mapping.length - 1].state.guard_applied == 3,
                     "D mapping endpoints are incorrect");
    result.harshness_mapping = mapping;

    requestAudio("presence_tone");
    release = readSeries(160, 50);
    failureDiagnostics = {
        window: "E_INITIAL_RELEASE",
        start_state: state,
        series: release
    };
    requireAdjacentGuard(release, "E initial release");
    requireGuardReleaseConfirmations(state, release, "E initial release");
    state = release[release.length - 1];
    requireCondition(state.guard_applied == 0 && state.guard_transition == 0,
                     "E initial release did not reach identity");
    for (index = 0; index < 12; index++) {
        requestAudio((index % 2) == 0 ?
                     "harshness_sweep_7db" : "harshness_sweep_10db");
        runFor(700);
        state = boardSnapshot();
        checkSafety(state, "E threshold jitter " + index);
        jitter.push(state);
    }
    requireAdjacentGuard(jitter, "E threshold jitter");
    requireCondition(jitter[jitter.length - 1].guard_level_changes -
                     jitter[0].guard_level_changes <
                     jitter[jitter.length - 1].guard_decisions -
                     jitter[0].guard_decisions,
                     "E Guard changed level on every decision");
    requestAudio("harshness_sweep_18db");
    runFor(5000);
    before = boardSnapshot();
    ...
    release = readSeries(160, 50);
    failureDiagnostics = {
        window: "E_SILENCE_RELEASE",
        start_state: before,
        series: release
    };
    requireAdjacentGuard(release, "E silence release");
    requireGuardReleaseConfirmations(before, release, "E silence release");
    state = release[release.length - 1];
    checkSafety(state, "E silence release");
    requireCondition(state.guard_applied == 0 && state.guard_transition == 0 &&
                     state.guard_processing == 0,
                     "E silence did not release Guard to identity");
    result.threshold_jitter = jitter;
    result.silence_release = release;
    result.silence_release_final = state;
    ...
    requestAudio("presence_tone");
    runFor(4000);
    combinedBefore = boardSnapshot();
    combinedBefore.gains = gainsSnapshot();
    write("EQ_DebugSmartBassEnabled = 1");
    write("EQ_DebugDynamicClarityEnabled = 1");
    write("EQ_DebugHarshnessGuardEnabled = 1");
    ...
    for (index = 0; index < 260; index++) {
        runFor(50);
        state = boardSnapshot();
        checkSafety(state, "F combined " + index);
        if (state.smart_applied > maxSmart) maxSmart = state.smart_applied;
        if (state.clarity_applied > maxClarity) maxClarity = state.clarity_applied;
        if (state.guard_applied > maxGuard) maxGuard = state.guard_applied;
        if ((index % 10) == 0) combinedSeries.push(state);
    }
    combinedAfter = boardSnapshot();
    combinedAfter.gains = gainsSnapshot();
    requireCondition(maxSmart == 4 && maxClarity == 4 && maxGuard == 3,
                     "F three dynamic modules did not reach MEDIUM ceilings");
    requireCondition(combinedAfter.builder_slices == combinedBefore.builder_slices &&
                     combinedAfter.control_request == combinedBefore.control_request &&
                     combinedAfter.control_applied == combinedBefore.control_applied,
                     "F dynamic modules changed builder or control tokens");
    sameGains(combinedBefore.gains, combinedAfter.gains,
              "F combined dynamics");
    result.combined_before = combinedBefore;
    result.combined_after = combinedAfter;
    result.combined_series = combinedSeries;
    result.combined_max_levels = {
        smart_bass: maxSmart, dynamic_clarity: maxClarity,
        harshness_guard: maxGuard
    };

    requestAudio("presence_tone");
    releaseBetween = readSeries(180, 50);
    state = releaseBetween[releaseBetween.length - 1];
    requireCondition(state.smart_applied == 0 && state.clarity_applied == 0 &&
                     state.guard_applied == 0,
                     "G dynamics did not release before overlap test");
    write("EQ_DebugMode = 0");
    runFor(3000);
    start = boardSnapshot();
    start.gains = gainsSnapshot();
    write("EQ_DebugFourWayTransitionArmMode = 1");
    requestAudio("all_dynamic_trigger");
    write("EQ_DebugAnalyzerResetRequest = 1");
    runFor(3500);
    state = boardSnapshot();
    checkSafety(state, "G FLAT to BASS uninterrupted");
    fourBass = state.four_way_overlap_count >
               start.four_way_overlap_count;
    requireCondition(fourBass && state.four_way_arm_count ==
                     start.four_way_arm_count + 1 &&
                     state.four_way_first_frame > start.process_frames &&
                     state.mode == 1 && state.eq_transition_target == -1,
                     "G FLAT to BASS fourfold overlap latch did not fire");
    events.push({window: "FLAT_TO_BASS", access_during_window: "NONE",
                 start: start, final_state: state});

    requestAudio("presence_tone");
    releaseBetween = readSeries(180, 50);
    state = releaseBetween[releaseBetween.length - 1];
    requireCondition(state.mode == 1 && state.smart_applied == 0 &&
                     state.clarity_applied == 0 && state.guard_applied == 0,
                     "G BASS precondition did not reach dynamic identity");
    before = state;
    write("EQ_DebugFourWayTransitionArmMode = 2");
    requestAudio("all_dynamic_trigger");
    write("EQ_DebugAnalyzerResetRequest = 1");
    runFor(3500);
    state = boardSnapshot();
    checkSafety(state, "G BASS to VOCAL uninterrupted");
    fourVocal = state.four_way_overlap_count >
                before.four_way_overlap_count;
    requireCondition(fourVocal && state.four_way_arm_count ==
                     before.four_way_arm_count + 1 &&
                     state.mode == 2 && state.eq_transition_target == -1,
                     "G BASS to VOCAL fourfold overlap latch did not fire");
    events.push({window: "BASS_TO_VOCAL", access_during_window: "NONE",
                 start: before, final_state: state});
    runFor(1000);
    finalState = boardSnapshot();
    finalState.gains = gainsSnapshot();
    checkSafety(finalState, "G final");
    sameGains([-2, -1, 0, 1, 2, 3, 2, 1, 0, -1],
              finalState.gains, "G VOCAL final gains");
    requirePresetControlDelta(start, finalState, 2, "G preset control");
    result.fourfold_transition = {
        flat_to_bass_overlap: fourBass,
        bass_to_vocal_overlap: fourVocal,
        events: events,
        raw_capture_during_overlap: "NOT_PERFORMED",
        start_state: start,
    ...
    write("EQ_DebugAnalyzerResetRequest = 1");
    runFor(6000);
    continuousStart = boardSnapshot();
    continuousStart.gains = gainsSnapshot();
    checkSafety(continuousStart, "I continuous start");
    requireCondition(continuousStart.analyzer_valid == 1 &&
                     continuousStart.analyzer_warmup == 1 &&
                     continuousStart.smart_decisions > 0 &&
                     continuousStart.clarity_decisions > 0 &&
                     continuousStart.guard_decisions > 0,
                     "I feature processing was not active before continuous run");
    hostStartNs = System.nanoTime();
    debugSession.target.runAsynch();
    Thread.sleep(301000);
    debugSession.target.halt();
    hostEndNs = System.nanoTime();
    continuousFinal = boardSnapshot();
    continuousFinal.gains = gainsSnapshot();
    checkSafety(continuousFinal, "I continuous final");
    hostElapsed = Number(hostEndNs - hostStartNs) / 1000000000.0;
    dspAudioSeconds = (continuousFinal.process_frames -
                       continuousStart.process_frames) * 1024.0 / 50000.0;
    requireCondition(hostElapsed >= 300.0 && dspAudioSeconds >= 300.0 &&
                     continuousFinal.smart_decisions >
                     continuousStart.smart_decisions &&
                     continuousFinal.clarity_decisions >
                     continuousStart.clarity_decisions &&
                     continuousFinal.guard_decisions >
                     continuousStart.guard_decisions,
                     "I did not complete 300 seconds of DSP audio");
    sameControlFingerprint(continuousStart, continuousFinal,
                           "I dynamic-only control", true);
    sameGains(continuousStart.gains, continuousFinal.gains,
              "I dynamic-only control");
    result.continuous_300s = {
        label: "CONTINUOUS_300S",
        target_wall_time_seconds: 300.0,
        measured_host_elapsed_seconds: hostElapsed,
        measured_dsp_audio_seconds: dspAudioSeconds,
        debugger_access_during_window: "NONE",
        start: continuousStart,
        final_state: continuousFinal
    };
    result.pass = true;
    result.dsp_left_running = true;
    return result;
}
...
try {
    ...
    Thread.sleep(500);
    saveResult(runHarshnessObjective());
    debugSession.target.runAsynch();
    Thread.sleep(500);
