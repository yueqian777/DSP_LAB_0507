/* Objective Dynamic Clarity transition capture and path timing. */
...
var resultPath = resultDir + "/dynamic_clarity_transition_board.json";
var script = null;
...
function saveCapture(label, baseLevel, targetLevel) {
    ...
    write("EQ_DebugDynamicClarityTransitionCaptureBaseLevel = " + baseLevel);
    write("EQ_DebugDynamicClarityTransitionCaptureTargetLevel = " + targetLevel);
    write("EQ_DebugDynamicClarityTransitionCaptureDone = 0");
    write("EQ_DebugDynamicClarityTransitionCaptureRequest = 1");
    runFor(3000);
    requireCondition(
        numberValue("EQ_DebugDynamicClarityTransitionCaptureDone") == 1,
        label + ": capture did not finish");
    requireCondition(numberValue("EQ_TriggerCaptureReady") == 1 &&
                     numberValue("EQ_TriggerCaptureSource") == 8,
                     label + ": trigger capture is invalid");
    requireCondition(
        numberValue("EQ_DebugDynamicClarityTransitionCaptureResult") >= 0,
...
function runObjective() {
    var result = {}, inputNames, shortNames, inputIndex, captures;
    ...
    result.path_frame_count = readArray(
        "EQ_DebugDynamicClarityTimingFrameCount", 5);
    result.path_last_cycles = readArray(
        "EQ_DebugDynamicClarityTimingLastCycles", 5);
    result.path_max_cycles = readArray(
        "EQ_DebugDynamicClarityTimingMaxCycles", 5);
    result.path_max_process_frame = readArray(
        "EQ_DebugDynamicClarityTimingMaxProcessFrame", 5);
    result.path_max_active_level = readArray(
        "EQ_DebugDynamicClarityTimingMaxActiveLevel", 5);
    result.path_max_pending_level = readArray(
        "EQ_DebugDynamicClarityTimingMaxPendingLevel", 5);
    result.path_max_target_level = readArray(
        "EQ_DebugDynamicClarityTimingMaxTargetLevel", 5);
    result.path_max_transition_remaining = readArray(
        "EQ_DebugDynamicClarityTimingMaxTransitionRemaining", 5);
    result.path_max_analyzer_count = readArray(
        "EQ_DebugDynamicClarityTimingMaxAnalyzerCount", 5);
    result.path_max_ad_frames = readArray(
        "EQ_DebugDynamicClarityTimingMaxAdFrames", 5);
    result.path_max_da_frames = readArray(
        "EQ_DebugDynamicClarityTimingMaxDaFrames", 5);
    result.path_max_flag_ad = readArray(
        "EQ_DebugDynamicClarityTimingMaxFlagAd", 5);
    result.path_max_flag_da = readArray(
        "EQ_DebugDynamicClarityTimingMaxFlagDa", 5);
    result.path_max_deadline = readArray(
        "EQ_DebugDynamicClarityTimingMaxDeadlineCount", 5);
    result.path_max_latency_miss = readArray(
        "EQ_DebugDynamicClarityTimingMaxLatencyMissCount", 5);
    result.path_max_overlap = readArray(
        "EQ_DebugDynamicClarityTimingMaxOverlapCount", 5);
    result.path_max_dropped = readArray(
        "EQ_DebugDynamicClarityTimingMaxDroppedCount", 5);
    result.path_max_update_count = readArray(
        "EQ_DebugDynamicClarityTimingMaxUpdateCount", 5);
    ...
    for (inputIndex = 0; inputIndex < 5; inputIndex++) {
        requireCondition(result.path_frame_count[inputIndex] > 0,
                         "Timing path was not exercised: " + inputIndex);
    }
    return result;
}
