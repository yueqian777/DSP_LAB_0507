...
static void ControlStimulus(int index, float *bass_db, float *rms_dbfs,
...
static int WriteControlCsv(const char *path, SMART_BASS_STATE *state)
{
    ...
    fprintf(file, "evidence_label,analysis_index,bass_relative_db,rms_dbfs,"
                  "requested_level,applied_level,transition_active,reason\n");
    ...
    for (analysis = 0; analysis < EVAL_CONTROL_STEPS; analysis++)
    {
        ...
        ControlStimulus(analysis, &bass_db, &rms_dbfs, &valid, &warmup);
        ...
        if (SmartBass_UpdateFromFeature(state, &snapshot) < 0)
        {
            fclose(file);
            return 0;
        }
        fprintf(file, "%s,%d,%.6f,%.6f,%d,%d,%d,%d\n",
                EVAL_EVIDENCE_LABEL, analysis, bass_db, rms_dbfs,
                state->target_level,
                state->active_level, state->transition_active,
                SmartBass_GetReason(state));

        for (frame_index = 0; frame_index < 8; frame_index++)
        {
            if (SmartBass_ProcessFrame(
                    state, frame, frame, SMART_BASS_MAX_FRAME_LEN) < 0)
            {
                fclose(file);
                return 0;
            }
...
int main(int argc, char **argv)
{
    ...
    if (argc != 3)
    {
        fprintf(stderr, "usage: smart_bass_response_eval response.csv control.csv\n");
        return 2;
    }
    SmartBass_Init(&state);
    if (state.initialized == 0)
    {
        fprintf(stderr, "SmartBass_Init failed\n");
        return 3;
    }
    ...
    printf("smart_bass_response_rows=%d control_rows=%d "
           "label=HOST_SIMULATED_CONTROL\n",
           SMART_BASS_LEVEL_COUNT * EVAL_RESPONSE_POINTS,
           EVAL_CONTROL_STEPS);
    return 0;
}
